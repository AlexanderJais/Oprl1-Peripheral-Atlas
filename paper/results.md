...
We measured the four opioid receptor transcripts in 19 neuronal populations from 12 mouse
peripheral ganglia, using whole-cell data throughout and a single analysis applied to every
dataset (Figure 1; Table S1). *Oprl1* is the highest-expressed of the four in 18 of the 19
...
the second receptor 44.41-fold in celiac neurons, 25.72-fold in the superior cervical ganglion,
and 14.69-fold in the thoracic chain, and exceeds *Oprm1* by 42- to 69-fold in the same
populations (Figures 1I to 1L). Four sympathetic ganglia from three laboratories on two platforms
agree, and every biological sample places *Oprl1* first when analyzed alone.
...
over the second receptor (Figure 1M). These neurons are cholinergic and not noradrenergic
...
is the second receptor rather than a trace transcript (Figure 1N).

Five of the populations in Figure 1 come from one experiment in which six autonomic ganglia were
dissected and sequenced together (GSE232789), which removes laboratory and platform from the
comparison between divisions. *Oprl1* is first in all five and its level varies only 1.7-fold
across them, from 16.33 CPM in the pelvic ganglion to 27.55 CPM in the stellate ganglion, while
the margin over the second receptor varies 11-fold. The margin therefore reports how far the other
three receptors fall in a given ganglion rather than how high *Oprl1* rises.

Enteric submucosal neurons place *Oprk1* first at postnatal day 7, at 122.51 CPM against *Oprl1*
26.70, a 4.59-fold lead in the opposite direction, in both samples at that age (Figure 1P). By
postnatal day 24 *Oprk1* has fallen to 3.77 CPM at 1.1% detection while *Oprl1* holds at 35.57
CPM, and *Oprl1* leads 2.20-fold in all three samples (Figure 1O). The two ages come from one
...
sit at comparable depth and give margins of 25.72-fold and 4.29-fold, and the five ganglia of
GSE232789 span an 11-fold range in margin at one depth and one chemistry.
